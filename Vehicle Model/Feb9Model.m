% Improved track profile for SHELL ECOMARATHON (updated 2.6.26)
%With hypermiling
% By Jordan Jeong and Dominic Padula.
% Mines 2026 Team

%This version (2/9/26 4:24pm) is the current working version
%It needs some hypermiling strategy tuning, as the model right now has
%commented out all constant speed cornering so everything is hypermiling.
%It does still account for slowing down for the required corners. But there
...
T_motor = 0.91; % [N-m] Torque output from the motor
GR = 9.23; % Gear Ratio
...
MotorControl_eff=0.95; %Good estimate from EE team
...
P_maxwh = Drivetrain_eff * P_max; % [W] available at wheels (approx)
...
vh_min= 19.5 /2.237;
vh_max=21.5/2.237;  %getting hypermiling stuff

vmax_firstgear=15/2.237;
...
while true
    ...
    % use out = motor_efficiency(T_load, speed_rpm, V_input
    % This value is for the transmission efficiency (change as you expect)
    %motorrpm=((v(i)*60)/(2*3.14159*r_t));
    if F>0.1
        j=j+1;
...
figure
histogram(MotorEff, 'BinLimits', [0 1], 'Normalization', 'count')
xlabel('Value')
ylabel('Count')
